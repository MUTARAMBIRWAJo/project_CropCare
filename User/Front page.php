<!DOCTYPE html>
<html lang="en">
<head>
    <title>CROP CARE | Home-Page</User-Home-Page></title>
    <link rel="stylesheet" href="User_Front_Style.css">
    <link rel="icon" href="CR Logo2.png" type="image/x-icon">
</head>
<body>
    <div class="header">
        <img src="CR Logo2.png" width="100px" height="100px">
        <h4>WELCOME TO YOUR PORTAL</h4>
        <span id="UserNames"><b>MUTARAMBIRWA Joseph</b></span>
        <ion-icon name="logo-user"></ion-icon>
    </div>
    <div class="content" style="display: flex; flex-wrap: wrap;">
    <div class="left-nav">
        <ul>
          <li><a href="YourInformation.php">Profile</a></li>
          <li><a href="Services.php"> Services</a></li>
          <li><a href="Report.php"> Report</a></li>
          <li><a href="Contract.php">Contract</a></li>
          <li><a href="c:\Users\JOSEPH\Desktop\PEOJECT FILE  AND REQUIRE\WEB PROJECT\htdocs\My Web Project/homepage.php">Log Out</a></li>
 
        </ul>
    </div>
    <div class="center-content" style="align-content: center; text-align: center;
    font-family: Arial, Helvetica, sans-serif;font-size: xx-large; border: 8px solid green;box-sizing: content-box;" >
        <p style="text-align: left;margin-top: 100px;margin-left: 20px; display: flex;">
            Welcome to yoour Account where you are going </br>to check on your activities,
             get updated on Prevous activities ,</br> Discount ,Request for service
            and check on your Dashboard.</br> Also on this Site you can meet with other Farmers and partners 
        </p>
    </div>
 
</div>
<footer style="background-color: black;">
  <tr bgcolor="black">
    <td colspan="6">
      <section style="display: flex;margin-left: 20px;">
      <div style="text-align: left;margin-top: 20px;" >
      <h4 style="color: rgb(23, 136, 8);"><b><u>PARTNERS LINKS :</u></b></h4>
      <a href="www.rab.gov.rw" style="color: aliceblue;">www.rab.gov.rw</a><br/>
      <a href="www.rab.gov.rw" style="color: aliceblue;">www.reg.gov.rw</a><br/>
      <a href="www.rab.gov.rw" style="color: aliceblue;">www.rema.gov.rw</a><br/>
      <p style="color: rgb(14, 53, 87);">With the help of these links you may visit <br> the websites of our partners to see 
        their activity <br> and explore much more of them</p>
      </div>
      <div style="margin-top: 20px;margin-left: 300px;">
        <h4 style="color: rgb(23, 136, 8);"><b><u>SUPPORTING LINKS:</u></b></h4>
        <a href="www:help.html" style="color: aliceblue;">Helps</a><br/>
       <a href="www.AboutUs.html" style="color: aliceblue;">About Us</a><br/>
       <a href="www.Faq.html" style="color: aliceblue;">FAQ</a><br/>
       <p style="color: rgb(14, 53, 87);"> If these links do not help you enough Please use <br>
         <mark> Contact Us </mark> on the top of this website</p>
      </div>
      <div style="margin-top: 20px;margin-left: 300px;">
        <h4 style="color: rgb(23, 136, 8);"><b><u>VISIT OUR SOCIAL MEDIA:</u></b></h4>
        <a href="http://www:facebook/Crop Care.com" style="color: aliceblue;">Facebook</a><br/>
       <a href="http://www.twitter/Crop Care.com" style="color: aliceblue;">Twitter</a><br/>
       <a href="http://www>instagram/Crop Care.com" style="color: aliceblue;">Instagram</a><br/>
       <p style="color: rgb(14, 53, 87);"> If these links do not help you enough Please use <br>
         <mark> Contact Us </mark> on the top of this website</p>
      </div>
    </section>
    </td>
    
  </tr>
</footer>
</body>
</html>